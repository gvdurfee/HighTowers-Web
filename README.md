...
5. **Optional: NAIP map patch overlay (Survey Location)**  
   Uses free USDA NAIP aerial imagery via the Node server (`GET /api/recent-imagery`). Requires `npm run dev:all` (or a deployed API with outbound HTTPS). No extra `.env` keys. See `server/README.md` and `docs/IMAGERY_OVERLAY_IMPLEMENTATION.md`.
