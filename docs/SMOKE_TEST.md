...
| A7 | **Export Reported Data** (`/export`) | Page loads; PDF generation **may require** hosted API (**`vars.VITE_API_BASE_URL`**) + CORS for static Mapbox proxy — note failures for `docs/API_HOSTING.md`. With a mission that has saved tower photos: uncheck **No content pack update**, upload a test ZIP, preview shows refine/append counts; download works when changes exist. |
| A7b | **Legacy redirect** (`/foreflight-content-pack`) | Redirects to `/export`. |
